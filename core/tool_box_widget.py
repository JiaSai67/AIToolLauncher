...
class ToolCardWidget(QWidget):
    ...
    stopRequested = Signal(dict)               # (tool_data)
    ...
    def show_context_menu(self, pos: QPoint):
        ...
        if self.is_installed:
            wdir = str(self.data.get("working_dir", "")).strip()
            name = str(self.data.get("name", "")).strip()
            is_local = (self.data.get("is_local") is True) or ("本地開發版" in name or "本地版" in name) or (wdir and "cloudtools" not in wdir.lower())
            ...
            if self.current_state == self.STATE_RUNNING:
                act_stop = Action(FluentIcon.CANCEL, "⏹️ 強制結束運行 (Force Stop)", triggered=lambda: self.stopRequested.emit(self.data))
